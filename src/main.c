...
void *pager_get_page(Pager *pager, uint32_t page_num) {
    if (page_num > TABLE_MAX_PAGES) {
        fprintf(stderr, "Page number %d out of bounds (max: %d).\n", page_num, TABLE_MAX_PAGES);
        exit(EXIT_FAILURE);
    }
...
void db_close(Table *table) {
    ...
    if (result == -1) {
        fprintf(stderr, "Error closing database file.\n");
        exit(EXIT_FAILURE);
    }
...
MetaResult meta_command(InputBuffer *input_buffer, Table *table) {
